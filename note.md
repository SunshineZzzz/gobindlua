...
程序员在代码中显式调用 runtime.GC()。这通常用于压力测试或某些特殊的内存释放场景。

#### 被动触发

根据 GOGC 变量（默认 100）决定。如果当前堆内存比上次 GC 结束后的存活内存增长了 100%，则触发 GC。

例如，若上次GC后存活对象为4MB，则下次GC将在堆内存达到8MB时触发。

#### 定时触发

即使程序内存增长非常缓慢，甚至完全没有新分配，Go 运行时也会有一个监控线程（sysmon）。

如果系统已经超过 2 分钟 没有进行过 GC，监控线程会强制触发一轮 GC。